...
void calculate_pid(int power) {
  error = (((S_LL + S_L) ) - (S_R + S_RR));
  P = error;
  I = I + previous_I;
  D = error - previous_error;
  PID_value = (0.25 * P) + (0.5 * I) + (2 * D);
  previous_I = I;
...
void calculate_pid_InCan(int power) {
  error = (((S_LL + S_L) ) - (S_R + S_RR));
  P = error;
  I = I + previous_I;
  D = error - previous_error;
  PID_value = (0.08 * P) + (0.5 * I) + (6 * D);
  previous_I = I;
  previous_error = error;
}
void Pid(int power) {
...
void Pid_InCan(int power) {
  calculate_pid_InCan(power);
  motor_control(power);
}
...
void InCan(int i, int can) {
  ...
  while (1) {
    if (S_LL > Ref_LL && S_L > Ref_L && S_C > Ref_C && S_R > Ref_R && S_RR > Ref_RR)
      {
        time++;
      }
    else
...
void PlaceCan(String s) {
  ...
  // เอามือลงก่่อนเข้าวาง
  // Stop(1);
  servo(Raise, Raise_Down + 10); // delay(200);
  ...
  while (1) {
    if ((S_LL < Ref_LL) && (S_RR < Ref_RR))
    {
      ...
      Stop(100);
      if ((i % 2) == 0) {
        motor(1, -35);  motor(2, 30); delay(70);
        Stop(20);
      } else {
        motor(1, 35);  motor(2, -30); delay(70);
        Stop(20);
...
int readCan(){ // คำสั่งอ่านค่ากระป๋องด้วยเซนเซอร์มือ ออกมาเป็นตัวเลข 1 ขาว 0 ดำ
  ...
  if (S_CR >= Ref_CR){
    return CanPosition[1]; // WHITE CAN
  }
  else{ 
    return CanPosition[0]; // YELLOW CAN
  }
}
void noCan(int can){
  Stop(100);
